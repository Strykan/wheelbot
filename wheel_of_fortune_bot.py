...
class Database:
    def __init__(self):
        self.conn = sqlite3.connect('wheel_of_fortune.db', check_same_thread=False)
        self.cursor = self.conn.cursor()
        self.init_db()
        
    def init_db(self):
        with self.conn:
            self.cursor.execute('''CREATE TABLE IF NOT EXISTS user_attempts
            ...
            self.cursor.execute('''CREATE TABLE IF NOT EXISTS prizes
                                  (id INTEGER PRIMARY KEY AUTOINCREMENT,
                                   user_id INTEGER NOT NULL,
            ...
            self.cursor.execute('''CREATE INDEX IF NOT EXISTS idx_transactions_status ON transactions(status)''')
            self.cursor.execute('''CREATE INDEX IF NOT EXISTS idx_prizes_user_id ON prizes(user_id)''')
...
def get_admin_payment_methods_keyboard():
    db.cursor.execute('SELECT id, name, is_active FROM payment_methods')
    methods = db.cursor.fetchall()
    keyboard = []
    for method in methods:
...
def get_admin_main_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📝 Неподтвержденные платежи", callback_data="pending_payments")],
        [InlineKeyboardButton("🔙 В меню", callback_data="back_to_start")]
    ])

# Функции работы с БД
def get_user_attempts(user_id):
    db.cursor.execute('SELECT paid, used, last_bonus_date FROM user_attempts WHERE user_id = ?', (user_id,))
...
def get_payment_methods():
    try:
        db.cursor.execute('SELECT id, name, details FROM payment_methods WHERE is_active = 1')
...
def add_payment_method(name, details):
    ...
    if not validate_input(name, 50) or not validate_input(details, MAX_INPUT_LENGTH):
        return False
        
    try:
        with db.conn:
            db.cursor.execute('SELECT id FROM payment_methods WHERE name = ?', (name,))
...
def create_transaction(user_id, amount, attempts, status='pending'):
    try:
        if amount > MAX_PAYMENT_AMOUNT:
            logger.warning(f"Попытка создать транзакцию с суммой {amount}, превышающей лимит")
            return None
...
def get_pending_transactions():
    try:
        db.cursor.execute('''SELECT t.id, t.user_id, t.amount, t.attempts, u.referral_code
                            FROM transactions t
                            LEFT JOIN user_attempts u ON t.user_id = u.user_id
...
async def admin_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        db.cursor.execute('SELECT COUNT(*) FROM user_attempts')
        total_users = db.cursor.fetchone()[0]
        
        db.cursor.execute('SELECT SUM(paid), SUM(used) FROM user_attempts')
        attempts = db.cursor.fetchone()
        total_paid = attempts[0] or 0
        total_used = attempts[1] or 0
        
        db.cursor.execute('''SELECT status, COUNT(*), SUM(amount) 
...
async def pending_payments(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for idx, (t_id, user_id, amount, attempts, ref_code) in enumerate(transactions, 1):
        text.append(
            f"{idx}. ID: {t_id}\n"
...
async def add_payment_method_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['admin_action'] = 'adding_payment_method'
    
    await query.message.edit_text(
        "✏️ <b>Добавление способа оплаты</b>\n\n"
...
async def edit_payment_method_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['admin_action'] = 'editing_payment_method'
    context.user_data['editing_method_id'] = method_id
    
    await query.message.edit_text(
        f"✏️ <b>Редактирование способа оплаты</b>\n\n"
        f"Текущее название: <code>{method[0]}</code>\n"
        f"Текущие реквизиты: <code>{method[1]}</code>\n\n"
        "Введите новые данные в формате:\n"
        "<code>Новое название\nНовые реквизиты</code>\n\n"
        "Пример:\n"
        "<code>Сбербанк\nНомер карты: 1234 5678 9012 3456</code>",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔙 Отмена", callback_data="manage_payment_methods")]
        ])
...
async def handle_admin_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if admin_action == 'adding_payment_method':
        ...
        context.user_data['new_payment_method'] = {'name': name}
        context.user_data['admin_action'] = 'adding_payment_details'
    ...
    elif admin_action == 'adding_payment_details':
        details = sanitize_input(update.message.text)
        ...
        payment_data = context.user_data.get('new_payment_method')
        if not payment_data:
            await update.message.reply_text(
        ...
        if add_payment_method(name, details):
            ...
            await update.message.reply_text(
                ...
                parse_mode=ParseMode.HTML,
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔙 В управление оплатой", callback_data="manage_payment_methods")]
                ])
            )
        else:
            await update.message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔙 Попробовать снова", callback_data="add_payment_method")]
                ])
            )
    
    elif admin_action == 'editing_payment_method':
        method_id = context.user_data.get('editing_method_id')
        if not method_id:
            await update.message.reply_text(
                "❌ Ошибка: не найден ID способа оплаты",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔙 Отмена", callback_data="manage_payment_methods")]
                ])
            )
        ...
        details = sanitize_input(details.strip())
        ...
        if update_payment_method(method_id, name, details):
            context.user_data.pop('admin_action', None)
            context.user_data.pop('editing_method_id', None)
            ...
            await update.message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔙 Попробовать снова", callback_data=f"edit_method_{method_id}")]
                ])
            )
...
async def check_attempts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prizes = get_unclaimed_prizes(user_id)
    ...
    if prizes:
        text.append("\n\n🏆 <b>Ваши неполученные призы:</b>")
        for prize_id, prize_type, value in prizes:
            text.append(f"▪️ {value} ({prize_type})")
...
async def daily_bonus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update_user_attempts(
        user_id=user_id,
        paid=DAILY_BONUS,
        last_bonus_date=datetime.now().isoformat()
    ):
        await query.answer("❌ Ошибка при начислении бонуса", show_alert=True)
        return
    ...
    await query.message.edit_text(
        ...
        f"🎯 Теперь у вас <b>{attempts['remaining']}</b> доступных попыток.",
        parse_mode=ParseMode.HTML,
        reply_markup=get_play_keyboard(user_id)
    )
...
async def show_payment_method(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text(
        ...
        f"❗️ В комментарии к платежу укажите ID: <code>{transaction_id}</code>",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup([
...
async def handle_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    amount, attempts = db.cursor.fetchone()
